...
struct Vert {
    int label;
    // for every node ---- prepare material space & world space
    Vec2 u; // material space
    Node* node; // world space

    // topological data拓扑数据
    std::vector<Face*> adjf; // adjacent faces相邻面
...
void apply_transformation_onto(const Mesh& start_state, Mesh& onto,  const Transformation& tr);
